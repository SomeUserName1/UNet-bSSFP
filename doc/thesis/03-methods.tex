...
A scanner consists of a primary superconductive electromagnetic coil, that is surrounded by a cryostat and thermal insulation, an excitation coil, spatial localization coils, and a receiver coil, as shown in \ref{scanner-teardown}.
...
Additionally, an \textbf{excitation system} is necessary to generate magnetic pulses $B_1(t)$ to cause a nutation as well as refocussing, spoiling, inversion and saturation in the region of interest.
...
\fig{img/nutation.png}{nutation}{A visualization of rotation $r$ (in our case we call it spin) in green, precession p in blue --- at the Larmor frequency --- and nutation n in red~\autocite{FilePraezessionsvgWikimediaCommons-2024-04-23}.}{0.2}
...
\fig{img/rot-frame.png}{rot-frame}{\textbf{(a)} An excitation pulse causes the magnetization towards the transverse plane in the laboratory frame. \textbf{(b)} The same phenomenon in the rotating frame. Note how the spiral with the $z$-axis as center is transformed into a geodesic~\autocite{nishimura}.}{0.8}
...
Frequency encoding is mutually exlusive with other simultaneous pulses.
...
\fig{img/mri-block-diagram.png}{mri-block-diag}{Block diagram of the components in an MRI scanner system~\autocite{Clare1997FunctionalM}.}{0.5}
...
\fig{img/gradient-echo.jpg}{gradient-echo}{Timing diagram of a gradient echo sequence~\autocite{hargreaves_rapid_2012}}{0.7}

On the other hand there are spin echo sequences, which re-phase by initially applying a $90^\circ$ pulse followed by $180^\circ$ pulses per $T_R$ rephasing pulse that flips the phase causing the dephasing factors (for example inhomogenieties) to rephase the spins.
...
On the other handside, the disturbance of the spheriocal oscillation causes dephasing.
...
In the case of free or isotropic diffusion the diffusion in all directions is similarly strong and will lead to a similar diffusion coefficient $D$.
...
\fig{img/pool.png}{pooling}{Max pooling (left) and average pooling (right) visualized for single channel 2D input from the previous layer.}{0.4}
...
\fig{img/unet.png}{unet}{The architecture of the UNet NN}{1}
...
\fig{img/i2i.png}{i2i}{Previously proposed methods for image to image translation~\autocite{pang_image-image_2021}.}{1}
...
\fig{img/birk.jpg}{birk}{The method employed by Birk et al.~\autocite{birk_high-resolution_2022}.}{1}
...
DWI was acquired in 4 sessions using an interleaved multi-slice single-shot SE-EPI DWI sequence with a $b$ value of $1000$ s$/$mm$^2$, a resolution of $2$mm isotropic, a $T_E$ of $62$ms, a $T_R$ of $3500$ms, a field of view of $220$mm, fat supression using saturation. \\
...
\fig{img/pc-bssfp-mag.png}{pc-bssfp-mag}{A data sample of the first channel containing the magnitude of the first phase cycle of the preprocessed pc-bSSFP images that is fed into the neural network.}{1.0}
\fig{img/pc-bssfp-pha.png}{pc-bssfp-pha}{A data sample of the second channel containing the phase of the first pahse cycle of the preprocessed pc-bSSFP images that is fed into the neural network.}{1.0}
...
\textbf{DWI} data was distortion corrected using FSL's~\autocite{smith_advances_2004} \texttt{TOPUP}, the brain was extracted using \texttt{BET} and Eddy currents were corrected with \texttt{eddy}.
...
\fig{img/dti-dxx.png}{dti-dxx}{Sample of the first channel of a preprocessed DTI corresponding to the $D_{xx}$ element of the tensor.}{1}
\fig{img/dti-dxy.png}{dti-dxy}{Sample of the second channel of a preprocessed DTI corresponding to the $D_{xy}$ element of the tensor.}{1}
...
\fig{img/t1w.png}{t1w}{Sample of a preprocessed $T_1$-weighted image fed to the NN.}{1}
...
\fig{img/augmentation.png}{augmentation}{Samples of the training data after data augmentation was applied (with a probability of $1$).}{1}
...
\fig{img/architecture.png}{arch}{The architecture of the NN used to predict DT from bSSFP.}{1}
