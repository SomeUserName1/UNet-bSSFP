...
class ConvBlock(tf.keras.layers.Layer):
    """
    Convolutional block with Group Normalization and PReLU activation
    including residual connection.

    Pre-activation ResNet block to help convergence and improve acc. as
    ...
    PReLU activation to avoid the vanishing gradient problem and improve
    convergence as proposed in the paper
    "Delving Deep into Rectifiers: Surpassing Human-Level Performance on
    ImageNet Classification" by Kaiming He et al.
    ...
    def __init__(self,
        ...
        for i in range(2):
            setattr(self, f'norm{i}',
                    layers.GroupNormalization(groups=groups, axis=-1))
            setattr(self, f'act{i}', layers.PReLU())
...
class BaseAttention(tf.keras.layers.Layer):
    """
    Base attention layer with multi-head attention, residual connection and
    layer normalization.
...
class CrossAttention(BaseAttention):
    """
    Cross attention layer with multi-head attention, residual connection and
    layer normalization.
    Taken from the official TensorFlow documentation:
    https://www.tensorflow.org/text/tutorials/transformer
...
class GlobalSelfAttention(BaseAttention):
    """
    Global self-attention layer with multi-head attention, residual connection
...
class SeqPool(layers.Layer):
    """
    Sequence pooling layer with a single dense layer and no bias as proposed
    in the paper
...
class EncoderBlock(tf.keras.layers.Layer):
    """
    Encoder block with convolutional block, global self-attention and max
    pooling.
...
class DecoderBlock(tf.keras.layers.Layer):
    def __init__(self, filters, kernel_size, groups, **kwargs):
        super().__init__(**kwargs)
        self.deconv = layers.Conv3DTranspose(filters,
                                             kernel_size=kernel_size,
                                             padding='same')
...
class FeedForward(tf.keras.layers.Layer):
    ...
    GELU activation function as proposed in the paper
    "Gaussian Error Linear Units (GELUs)" by Dan Hendrycks et al.
...
# CCT uses 2 layer transformer encoder with 2 heads and 64 dims per head
# and 128 mlp_dim
class TransformerBlock(tf.keras.layers.Layer):
    def __init__(self, d_model, num_heads, mlp_dim, dropout=0.1):
        super().__init__()
        self.ln = tf.keras.layers.LayerNormalization(epsilon=1e-6)
        self.gsa = GlobalSelfAttention(d_model,
                                       num_heads,
                                       mlp_dim,
                                       dropout=dropout)
...
class DemonAdamUpdateCallback(tf.keras.callbacks.Callback):
    """
    Usage:
    ```
    ...
    model.fit(train_ds, callbacks=[ DemonAdamUpdate(beta_1=beta_1,
                                                    total_steps=10000)])
    ```
    """
    ...
    def __init__(self,
                 ...
                 total_steps: int,
                 beta_init: float = 0.9):
        super(DemonAdamUpdateCallback, self).__init__()
        self.beta_1 = beta_1
    ...
    def on_batch_end(self, batch, logs=None):
        ...
        beta = self.beta_init * decay / ((1. - self.beta_init)
                                         + self.beta_init * decay)
